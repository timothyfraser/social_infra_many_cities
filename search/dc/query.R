...
mybox <- getbb(place_name = "Washington, DC")
...
mybounds  %>%
  ...
  st_write("dc/bounds.geojson", delete_dsn = TRUE)

# Import city bounds
mybounds <- read_sf("dc/bounds.geojson")
...
get_data(name = "dc", state = "DC",
         county = "District of Columbia",
         csub =  c("Washington"))

# Use city-provided bounds from above
#get_bounds(name = "dc") # Obtain relevant boundaries for study area
get_grid(name = "dc") # Obtain grid


# Run 5 Test Searches
get_testapi(name = "dc", yourkey = mykey, parallel = TRUE)
read_rds("dc/test_sites.rds") # It works!
get_results(name = "dc", test = TRUE) # Refine results
get_map(name = "dc", test = TRUE)

# Run actual searches
get_api(name = "dc", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "dc", test = FALSE) # Refine results
get_map(name = "dc", test = FALSE)
get_diagnostics(name = "dc")
get_grid_1km(name = "dc")

read_sf("dc/grid.geojson") %>% st_area() %>% sum() / 1e6
```


# 2. PARAMETERS #########################################
name = get_folder("dc") # Get Folder-Augmented Name (eg. search/nyc)
