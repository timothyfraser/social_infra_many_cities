#' @name oklahoma/query.R
#' @title API queries for: oklahoma
...
mybox <- getbb(place_name = "Oklahoma City, OK")
...
mybounds  %>%
  filter(name == "Oklahoma City") %>%
  st_transform(crs = wgs)  %>%
  summarize(geometry = st_union(geometry)) %>%
  st_make_valid() %>%
  sfheaders::sf_remove_holes() %>%
  st_write("oklahoma/bounds.geojson", delete_dsn = TRUE)

# Import city bounds
mybounds <- read_sf("oklahoma/bounds.geojson")

test <- tigris::county_subdivisions(
  state = "OK",
  county = c("Oklahoma County", "Canadian County", "Cleveland County",
             "Pottawatomie County"),
  cb = TRUE, year = 2019) %>%
  st_as_sf() %>%
...
get_data(name = "oklahoma", state = "OK",
         county = c("Oklahoma County", "Canadian County", "Cleveland County",
                    "Pottawatomie County"),
         csub =  c("East Canadian", "East Cleveland", "El Reno",
                   "Norman", "North Cleveland", "Oklahoma City Northeast",
                   "Oklahoma City Northwest", "Oklahoma City Southeast",
                   "Oklahoma Southwest", "Shawnee"))

# Use city-provided bounds from above
#get_bounds(name = "oklahoma") # Obtain relevant boundaries for study area
get_grid(name = "oklahoma") # Obtain grid


# Run 5 Test Searches
get_testapi(name = "oklahoma", yourkey = mykey, parallel = TRUE)
read_rds("oklahoma/test_sites.rds")$vicinity # It works!
get_results(name = "oklahoma", test = TRUE) # Refine results
get_map(name = "oklahoma", test = TRUE)

read_sf("oklahoma/grid.geojson") %>% st_area() %>% sum() / 1e6

# Run actual searches
get_api(name = "oklahoma", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "oklahoma", test = FALSE) # Refine results
get_map(name = "oklahoma", test = FALSE)
get_diagnostics(name = "oklahoma")
get_grid_1km(name = "oklahoma")
...
# 2. PARAMETERS #########################################
name = get_folder("oklahoma") # Get Folder-Augmented Name (eg. search/nyc)
