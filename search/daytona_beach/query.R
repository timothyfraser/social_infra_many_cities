#' @name daytona_beach/query.R
#' @title API queries for: daytona_beach
...
```{r}
dir.create("daytona_beach")

library(osmdata)

mybox <- getbb(place_name = "Daytona Beach, FL")
...
mybounds  %>%
  filter(name == "Daytona Beach") %>%
  st_transform(crs = wgs)  %>%
  summarize(geometry = st_union(geometry)) %>%
  st_make_valid() %>%
  sfheaders::sf_remove_holes() %>%
  st_write("daytona_beach/bounds.geojson", delete_dsn = TRUE)


# Import city bounds
mybounds <- read_sf("daytona_beach/bounds.geojson")

test <- tigris::county_subdivisions(
  state = "FL",
  county = c("Volusia County"),
  cb = TRUE, year = 2019) %>%
  st_as_sf() %>%
...
get_data(name = "daytona_beach", state = "FL",
         county = c("Volusia County"),
         csub =  c("Daytona Beach"))
# Use city-provided bounds from above
#get_bounds(name = "daytona_beach") # Obtain relevant boundaries for study area
get_grid(name = "daytona_beach") # Obtain grid

read_sf("daytona_beach/grid.geojson") %>% st_area() %>% sum() / 1e6

# Run 5 Test Searches
get_testapi(name = "daytona_beach", yourkey = mykey, parallel = TRUE)
read_rds("daytona_beach/test_sites.rds") # It works!
get_results(name = "daytona_beach", test = TRUE) # Refine results
get_map(name = "daytona_beach", test = TRUE)


# Run actual searches
get_api(name = "daytona_beach", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "daytona_beach", test = FALSE) # Refine results
get_map(name = "daytona_beach", test = FALSE)
get_diagnostics(name = "daytona_beach")
get_grid_1km(name = "daytona_beach")
...
# 2. PARAMETERS #########################################
name = get_folder("daytona_beach") # Get Folder-Augmented Name (eg. search/nyc)
