#' @name nashville/query.R
#' @title API queries for: nashville
...
```{r}
dir.create("nashville")

library(osmdata)

mybox <- getbb(place_name = "Nashville, TN")
...
mybounds  %>%
  filter(name == "Nashville-Davidson") %>%
  st_transform(crs = wgs)  %>%
  summarize(geometry = st_union(geometry)) %>%
  st_make_valid() %>%
  sfheaders::sf_remove_holes() %>%
  st_write("nashville/bounds.geojson", delete_dsn = TRUE)

# Import city bounds
mybounds <- read_sf("nashville/bounds.geojson")
...
get_data(name = "nashville", state = "TN",
         county = "Davidson County",
         csub =  c(1:35) %>% as.character())

# Use city-provided bounds from above
#get_bounds(name = "nashville") # Obtain relevant boundaries for study area
get_grid(name = "nashville") # Obtain grid


# Run 5 Test Searches
get_testapi(name = "nashville", yourkey = mykey, parallel = TRUE)
read_rds("nashville/test_sites.rds") # It works!
get_results(name = "nashville", test = TRUE) # Refine results
get_map(name = "nashville", test = TRUE)

#read_sf("nashville/grid.geojson") %>% st_area() %>% sum() / 1e6

# Run actual searches
get_api(name = "nashville", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "nashville", test = FALSE) # Refine results
get_map(name = "nashville", test = FALSE)
get_diagnostics(name = "nashville")
get_grid_1km(name = "nashville")
...
# 2. PARAMETERS #########################################
name = get_folder("nashville") # Get Folder-Augmented Name (eg. search/nyc)
