#' @name louisville/query.R
#' @title API queries for: louisville
...
```{r}
dir.create("louisville")

library(osmdata)

test <- tigris::county_subdivisions(
  state = "KY",
  county = c("Jefferson County"),
  cb = TRUE, year = 2019) %>%
  st_as_sf() %>%
...
test %>%
  filter(str_detect(name, "Louisville")) %>%
  st_make_valid() %>%
  summarize(geometry = st_union(geometry)) %>%
  st_make_valid() %>%
  sfheaders::sf_remove_holes() %>%
  st_write("louisville/bounds.geojson", delete_dsn = TRUE)

# Import city bounds
mybounds <- read_sf("louisville/bounds.geojson")
...
get_data(name = "louisville", state = "KY",
         county = "Jefferson County",
         csub =  c(mynames))
# Use city-provided bounds from above
#get_bounds(name = "louisville") # Obtain relevant boundaries for study area
get_grid(name = "louisville") # Obtain grid

read_sf("louisville/grid.geojson") %>% st_area() %>% sum() / 1e6

# Run 5 Test Searches
get_testapi(name = "louisville", yourkey = mykey, parallel = TRUE)
read_rds("louisville/test_sites.rds") # It works!
get_results(name = "louisville", test = TRUE) # Refine results
get_map(name = "louisville", test = TRUE)


# Run actual searches
get_api(name = "louisville", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "louisville", test = FALSE) # Refine results
get_map(name = "louisville", test = FALSE)
get_diagnostics(name = "louisville")
get_grid_1km(name = "louisville")
...
# 2. PARAMETERS #########################################
name = get_folder("louisville") # Get Folder-Augmented Name (eg. search/nyc)
