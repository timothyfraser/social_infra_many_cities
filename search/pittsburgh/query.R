#' @name pittsburgh/query.R
#' @title API queries for: pittsburgh
...
```{r}
dir.create("pittsburgh")

library(osmdata)

mybox <- getbb(place_name = "Pittsburgh, PA")
...
mybounds %>%
  ...
  st_write("pittsburgh/bounds.geojson", delete_dsn = TRUE)


# Import city bounds
mybounds <- read_sf("pittsburgh/bounds.geojson")
...
get_data(name = "pittsburgh", state = "PA",
         county = c("Allegheny County"),
         csub =  c(mynames))
# Use city-provided bounds from above
#get_bounds(name = "pittsburgh") # Obtain relevant boundaries for study area
get_grid(name = "pittsburgh") # Obtain grid

read_sf("pittsburgh/grid.geojson") %>% st_area() %>% sum() / 1e6

# Run 5 Test Searches
get_testapi(name = "pittsburgh", yourkey = mykey, parallel = TRUE)
read_rds("pittsburgh/test_sites.rds") # It works!
get_results(name = "pittsburgh", test = TRUE) # Refine results
get_map(name = "pittsburgh", test = TRUE)


# Run actual searches
get_api(name = "pittsburgh", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "pittsburgh", test = FALSE) # Refine results
get_map(name = "pittsburgh", test = FALSE)
get_diagnostics(name = "pittsburgh")
get_grid_1km(name = "pittsburgh")
...
# 2. PARAMETERS #########################################
name = get_folder("pittsburgh") # Get Folder-Augmented Name (eg. search/nyc)
