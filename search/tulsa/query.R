...
test <- tigris::county_subdivisions(
  state = "OK",
  county = c("Osage County", "Rogers County", "Tulsa County", "Wagoner County"),
  cb = TRUE, year = 2019) %>%
  st_as_sf() %>%
...
test %>%
  ...
  st_write("tulsa/bounds.geojson", delete_dsn = TRUE)

# Import city bounds
mybounds <- read_sf("tulsa/bounds.geojson")
...
get_data(name = "tulsa", state = "OK",
         county = c("Osage County", "Rogers County", "Tulsa County", "Wagoner County"),
         csub =  c(mynames))
# Use city-provided bounds from above
#get_bounds(name = "tulsa") # Obtain relevant boundaries for study area
get_grid(name = "tulsa") # Obtain grid

read_sf("tulsa/grid.geojson") %>% st_area() %>% sum() / 1e6

# Run 5 Test Searches
get_testapi(name = "tulsa", yourkey = mykey, parallel = TRUE)
read_rds("tulsa/test_sites.rds") # It works!
get_results(name = "tulsa", test = TRUE) # Refine results
get_map(name = "tulsa", test = TRUE)


# Run actual searches
get_api(name = "tulsa", yourkey = mykey, parallel = TRUE) # Run Searches


get_results(name = "tulsa", test = FALSE) # Refine results
get_map(name = "tulsa", test = FALSE)
get_diagnostics(name = "tulsa")
get_grid_1km(name = "tulsa")
...
# 2. PARAMETERS #########################################
name = get_folder("tulsa") # Get Folder-Augmented Name (eg. search/nyc)
