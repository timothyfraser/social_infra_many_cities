...
v = c("pop_density", "white", "black", "asian", "hisplat",
...
raw = geo %>%
  ...
  select(geoid, name, all_of(v$var))
  # Transform variables like in model
  # mutate(
  #   pop_density = log(pop_density),
  #   white = logit(white, 0.01),
  #   black = logit(black, 0.01),
  #   hisplat = logit(hisplat, 0.01),
  #   asian = logit(asian, 0.01),
  #   median_household_income = log(median_household_income),
  #   income_0_60K= logit(income_0_60K, 0.01),
  #   some_college = logit(some_college, 0.01),
  #   unemployment = log(unemployment + 1)
  # )
...
transform_it = function(var, value, .f = "log"){
  ...
  case_when(
    ...
    var == "pop_density" ~ .f(value + k*1) - j*1,
    # For percentages and rates, add just 1% (so it's not 0)
    var == "hisplat" ~ .f(value + k*0.01) - j*0.01,
    var == "black" ~ .f(value + k*0.01) - j*0.01,
    var == "asian" ~ .f(value + k*0.01) - j*0.01,
    var == "unemployment" ~ .f(value + k*0.01) - j*0.01,
    TRUE ~ value)
}
...
mutate(x1 = transform_it(var = var, value = x1, .f = "exp"),
  ...
  mutate(
    across(.cols = c(x1, x2, xdiff),
           .fns = ~case_when(
             var %in% c("white", "hisplat", "black", "asian",
                        "over_65", "income_0_60K", "some_college") ~ scales::percent(.x, accuracy = 1, suffix = "%", style_positive = "plus", style_negative = "minus"),
             var == "median_household_income" ~ scales::number(.x, accuracy = 1, prefix = "$", decimal.mark = ",", style_positive = "plus", style_negative = "minus"),
...
colors$b2 = colorspace::adjust_transparency(colors$blue, alpha = 0.5)
colors$b3 = colorspace::adjust_transparency(colors$blue, alpha = 0.25)
...
ylab = paste0("Value Range (n = ", n_cells, " tracts)",
              "\n", "(standard deviations, where 0 = mean)")
title = paste0("Difference in Mean Demographic Traits \nfor Tracts in Boston vs. Other Top 24 Cities")
...
gg = ggplot() +
  ...
  shadowtext::geom_shadowtext(
    mapping = aes(x = benchmark, y = -5, label = "Distribution"),
...
ggsave(plot = gg, filename = "viz/figure_3a_crossbars_boston_vs_range.png", dpi = 500, width = 9, height = 8)
browseURL("viz/figure_3a_crossbars_boston_vs_range.png")
dbDisconnect(geo)
rm(list = ls()); gc()
